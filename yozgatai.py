...
try:
    ...
    st.error(f"Ayar hatası: {e}"); st.stop()
        ...
        if st.button("Sisteme Gir"):
            ...
            if not df.empty:
                ...
                if basarili:
                    ...
                    st.success("Roket kalkıyor... 🚀"); time.sleep(1); st.rerun()
                else: st.error("Gardaşım uyuşmadı.")
    with tab2: st.link_button("📝 Kayıt Formuna Git", KAYIT_FORM_VIEW)
    st.stop()
...
with st.sidebar:
    ...
    if st.button("Çıkış Yap"): st.session_state.oturum = None; st.rerun()
...
if soru := st.chat_input("Emmiye sor hele..."):
    ...
    try: requests.post(CHAT_FORM_URL, data={ENTRY_CHAT_USER: st.session_state.oturum, ENTRY_CHAT_MSG: soru, ENTRY_CHAT_ROLE: "user"})
    except: pass

    try:
        # 🕵️‍♂️ AKILLI MODEL SEÇİMİ (404 SAVAR)
        try: model = genai.GenerativeModel('gemini-1.5-flash')
        except: model = genai.GenerativeModel('gemini-pro')
        
        prompt = f"Sen Yozgatlı, bilge ve şiveli konuşan bir emmisin. Adın YozgatAI. Şiveli cevap ver: {soru}"
    ...
    except Exception as e: st.error(f"Emmi dalgın: {e}")